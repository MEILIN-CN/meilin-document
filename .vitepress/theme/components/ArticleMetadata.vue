...
const readTime = computed(() => {
    if (wordCount.value === 0 && imageCount.value === 0) return 0
...
function analyze() {
    ...
    const contentClone = docDomContainer.cloneNode(true) as HTMLElement;

    // Remove the metadata component itself from the clone to avoid counting its words
    contentClone.querySelectorAll('.word').forEach(el => el.remove());

    const imgs = contentClone.querySelectorAll<HTMLImageElement>('img');
    imageCount.value = imgs?.length || 0;

    const words = contentClone.textContent || '';
...
<template>
    ...
    <div class="word" v-if="readTime > 0">
        <p>
